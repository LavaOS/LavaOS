...
int main(int argc, char **argv) {
    ...
    if(!nob_mkdir_if_not_exists_silent(temp_sprintf("%s/time", bindir))) return 1;
    const char* output = temp_sprintf("%s/time/time", bindir);
    File_Paths pathb = { 0 };
    ...
    if(nob_c_needs_rebuild(&stb, &pathb, output, sources, ARRAY_LEN(sources))) {
        ...
        cmd_append(&cmd, temp_sprintf("-L%s/bin/librtc", minos_root));
        cmd_append(&cmd, "-lrtc");
        cmd_append(&cmd, "-I../librtc/include");
        da_append_many(&cmd, sources, ARRAY_LEN(sources));
        if(!nob_cmd_run_sync_and_reset(&cmd)) return 1;
    }
    char* rootdir = getenv("ROOTDIR");
    if(rootdir && !copy_file(output, temp_sprintf("%s/user/time", rootdir))) 
        return 1;
}
